...
import { Card, Text, IconButton, Avatar, Tooltip } from "react-native-paper";
...
const Notes = ({ firebaseAuth }: props) => {
    ...
    const { data: user, status: userStatus, refetch: refetchUser } = useUser(firebaseAuth);
    // pass user to useNotes. If user is null, useNotes will return null
    const { data: notes, status: notesStatus, refetch: refetchNotes } = useNotes(user || null);
    ...
    }, [firebaseAuth, refetchUser, refetchNotes]);
